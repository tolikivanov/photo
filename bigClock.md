...
Показать время, цену, курс рубля, любую информацию, которая состоит из цифр, можно с помощью семисегментных индикаторов. Они хорошо читаются даже на большом растоянии и просты в управлении.

Как "скрафтить" из картона и светодиодной ленты, по-настоящему большие индикаторы и запустить на них часы я расскажу в этой статье.
...
На одну из сторон, теперь она будет тыльной, приклем дополнительные "прямоугольнички" вырезанные из того же картона вдоль рёбер жесткости. Сквозь них пропустим провода.

![Add wire chanel](https://github.com/tolikivanov/photo/raw/master/Wire-chanel.jpg)

Для подсветки сегмента будущего индикатора хватит 10 см отрезка светодиодной ленты. Припаяем к нему пару проводов.

![Add wire](https://github.com/tolikivanov/photo/raw/master/led-wire.jpg)

Рассеивателем для ленты станет цилиндр, вырезанный и свёрнутый из обычной офисной бумаги А4, склеенной клеем карандашом.
...
Торцы цилинров закроем заглушками вырезанными всё из того же картона.

![Add stub](https://github.com/tolikivanov/photo/raw/master/stub.jpg)

Тоже самое осталось провернуть ещё для 3-х цифр.
...
Чтобы управлять одним индикатором понадобиться 7 выходов микроконтроллера. Четыри индикатора и разделитель - это 29, а у Ардуино Уно выходов всего 20.
...
Хоть я и размахнулся с размер будущих часов, но схема на отдельных модулях - это слишком громоздко.

![A lot of Fets](https://github.com/tolikivanov/photo/raw/master/a-lot-of-fets.jpg)

### Решение

Гораздо более элегантно будет смотреться модуль OctoFet - готовая сборка из сдвигового регистра и 8-ми полевиков, каждый из которых расчитан на напряжение до 30 вольт и может пропустить через себя до 3-х ампер. 
...
Сам Octofet, чтоб не болтался, закрепим на основании, которое можно вырезать всё из того же картона.
...
Прошьем контроллер прошивкой для вывода текущего времени.
...
Это лишь пример того, как из подручных материалов и разыгравшейся фантазии можно сделать полноценный проект.
Добавьте сюда модуль Bluetooth для управление и настройки часов или звуковое оповещение чтобы превратить часы в будильник.
